...
subject_dict = ({})  # словарь для хранения пользовательской информации из разных чатов, ключ = chat.id, а значение - выбранный предмет
deadline_dict = ({})  # словарь для хранения пользовательской информации из разных чатов, ключ = chat.id, а значение - номер дедлайна
...
def is_valid_date(date: str = "01/01/00", divider: str = "/") -> bool:
    ...
    try:
        deadline_date = datetime.striptime(date, f"%d{divider}%m{divider}%y")

        if deadline_date.date() < datetime.today().date():
            return False
        if deadline_date.date() > (datetime.today() + timedelta(days=365)).date():
            return False
        if deadline_date.date() > datetime.today().date():
            calendar_date = deadline_date.date()
            datetime(calendar_date.year, calendar_date.month, calendar_date.day)
...
def connect_table(message):
    ...
    url = message.text
    sheet_id = "15L-ZkV96TSxG4The10aiuEWh9fTbvu_FKZW3c7JTi-E" # Нужно извлечь id страницы из ссылки на Google-таблицу
    try:
...
def choose_action(message):
    ...
    if message.text == "Подключить Google-таблицу":
        ...
        week = date_today + timedelta(days=7)
        dead = ""
        for strok in range(
            ...
            for current_d in worksheet.row_values(strok)[2:]:  # проходимся по ряду
                deadline_date = convert_date(current_d)
                if deadline_date != False:
                    if week >= deadline_date >= date_today:
                        dead += (
                                worksheet.cell(strok, 1).value + " : " + current_d + "\n"
                        )  # предмет и дата дедлайна
        info = bot.send_message(message.chat.id, dead)
        start(message)
        return
    else:
        info = bot.send_message(
